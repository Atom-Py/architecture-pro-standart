...
Партнёрский кол-центр находится вне контура банка и не может вызывать API, поэтому для него ставки выкладываются файлом. Для этого в платформе депозитов появляется новый контейнер - сервис публикации ставок. Он формирует полный срез в CSV вместе с манифестом и кладёт его на SFTP-сервер банка в демилитаризованной зоне, откуда партнёр забирает файл по своему расписанию.

По сравнению с решением MVP состав систем меняется в трёх местах: в платформе депозитов добавляется сервис публикации ставок, в системе кол-центра расширяется существующий адаптер, который в MVP умел только принимать заявки с сайта, и в инфраструктуре появляется SFTP-сервер в демилитаризованной зоне. Остальные контейнеры используются без изменений.
